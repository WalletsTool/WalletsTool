...
// 懒加载非关键组件
const ChainManagement = defineAsyncComponent(
    () => import("@/components/ChainManagement.vue"),
);
const RpcManagement = defineAsyncComponent(
    () => import("@/components/RpcManagement.vue"),
);
const TokenManagement = defineAsyncComponent(
    () => import("@/components/TokenManagement.vue"),
);
const WalletImportModal = defineAsyncComponent(
    () => import("@/components/WalletImportModal.vue"),
);
const ProxyConfigModal = defineAsyncComponent(
    () => import("@/components/ProxyConfigModal.vue"),
);
  ...
  return {
    ...
    delay: [
      form.min_interval && form.min_interval.trim() !== ""
          ? Number(form.min_interval)
          : 1,
      form.max_interval && form.max_interval.trim() !== ""
          ? Number(form.max_interval)
          : 3,
    ],
    transfer_type: form.send_type,
    transfer_amount_list: [
      form.send_min_count && form.send_min_count.trim() !== ""
          ? Number(form.send_min_count)
          : 0,
      form.send_max_count && form.send_max_count.trim() !== ""
          ? Number(form.send_max_count)
          : 0,
    ],
    left_amount_list: [
      form.send_min_count && form.send_min_count.trim() !== ""
    ...
    amount_precision:
        form.amount_precision && form.amount_precision.trim() !== ""
            ? Number(form.amount_precision)
            : 6,
    limit_type: form.limit_type,
    limit_count:
        form.limit_count && form.limit_count.trim() !== ""
            ? Number(form.limit_count)
            : 21000,
    limit_count_list: [
      form.limit_min_count && form.limit_min_count.trim() !== ""
          ? Number(form.limit_min_count)
          : 21000,
      form.limit_max_count && form.limit_max_count.trim() !== ""
          ? Number(form.limit_max_count)
          : 30000,
    ],
    gas_price_type: form.gas_price_type,
    gas_price_rate:
        form.gas_price_rate && form.gas_price_rate.trim() !== ""
            ? Number(form.gas_price_rate) / 100
            : 0.05,
    gas_price:
        form.gas_price && form.gas_price.trim() !== ""
            ? Number(form.gas_price)
            : 30,
    max_gas_price:
        form.max_gas_price && form.max_gas_price.trim() !== ""
            ? Number(form.max_gas_price)
            : 0,
  };
});
...
const transferStatistics = computed(() => {
  ...
  const pending = data.value.filter(
      (item) => item.exec_status === "0",
  ).length;
  const processing = data.value.filter(
      (item) => item.exec_status === "1",
  ).length;
  const succeeded = data.value.filter(
...
const filteredTransferData = computed(() => {
  if (
      !filterForm.platBalanceValue &&
      !filterForm.coinBalanceValue &&
      !filterForm.errorMsg
  ) {
    return data.value;
  }
  ...
  return data.value.filter((item) => {
    ...
    if (
        filterForm.platBalanceValue &&
        filterForm.platBalanceValue.trim() !== ""
    ) {
      const platBalanceValue = parseFloat(filterForm.platBalanceValue);
      const itemPlatBalance = parseFloat(item.plat_balance || 0);

      if (
          filterForm.platBalanceOperator === "gt" &&
          itemPlatBalance <= platBalanceValue
      ) {
        return false;
      } else if (
          filterForm.platBalanceOperator === "eq" &&
          itemPlatBalance !== platBalanceValue
      ) {
        return false;
      } else if (
          filterForm.platBalanceOperator === "lt" &&
          itemPlatBalance >= platBalanceValue
      ) {
        return false;
      }
    ...
    if (
        filterForm.coinBalanceValue &&
        filterForm.coinBalanceValue.trim() !== ""
    ) {
      const coinBalanceValue = parseFloat(filterForm.coinBalanceValue);
      const itemCoinBalance = parseFloat(item.coin_balance || 0);

      if (
          filterForm.coinBalanceOperator === "gt" &&
          itemCoinBalance <= coinBalanceValue
      ) {
        return false;
      } else if (
          filterForm.coinBalanceOperator === "eq" &&
          itemCoinBalance !== coinBalanceValue
      ) {
        return false;
      } else if (
          filterForm.coinBalanceOperator === "lt" &&
          itemCoinBalance >= coinBalanceValue
      ) {
        return false;
      }
    ...
    if (filterForm.errorMsg && filterForm.errorMsg.trim() !== "") {
      ...
      if (
          !errorMsg
              .toLowerCase()
              .includes(filterForm.errorMsg.toLowerCase())
      ) {
        return false;
      }
    }
  ...
  try {
    ...
    for (const {configKey, windowId, windowLabel} of configKeys) {
      ...
      webview.once("tauri://created", () => {
        openedCount++;
        Notification.success({
          content: `已打开新窗口: 批量转账 ${windowId} (${openedCount}/${windowCount})`,
          position: "topLeft",
        });
...
function applySharedConfig(config) {
  ...
  try {
    ...
    if (config.chainValue) {
      ...
      // 找到对应的链对象
      const chain = chainOptions.value.find(
          (c) => c.key === config.chainValue,
      );
      if (chain) {
    ...
    if (config.coinValue) {
      ...
      // 找到对应的币种对象
      const coin = coinOptions.value.find(
          (c) => c.key === config.coinValue,
      );
      if (coin) {
...
function updateTransferProgress() {
  ...
  const completed = data.value.filter(
      (item) => item.exec_status === "2" || item.exec_status === "3",
  ).length;

  transferCompleted.value = completed;
  // 计算进度百分比
  if (transferTotal.value > 0) {
    transferProgress.value = Number(
        (completed / transferTotal.value).toFixed(2),
    );
  } else {
...
function updateBalanceProgress() {
  ...
  if (balanceTotal.value > 0) {
    balanceProgress.value = Number(
        (completed / balanceTotal.value).toFixed(4),
    );
  } else {
...
function updateToAddressBalanceProgress() {
  ...
  const completed = itemsWithToAddr.filter(
      (item) =>
          (item.plat_balance !== "" && item.plat_balance !== null) ||
          (item.coin_balance !== "" && item.coin_balance !== null) ||
          item.exec_status === "3",
  ...
  if (toAddressBalanceTotal.value > 0) {
    toAddressBalanceProgress.value = Number(
        (completed / toAddressBalanceTotal.value).toFixed(4),
    );
  } else {
  ...
  // 如果全部完成，延迟隐藏进度条
  if (
      completed === toAddressBalanceTotal.value &&
      toAddressBalanceTotal.value > 0
  ) {
    setTimeout(() => {
...
async function sendTip() {
  ...
  try {
    ...
    const config = {
      ...
      gas_price_type: form.gas_price_type || "1",
      gas_price_rate:
          form.gas_price_rate && form.gas_price_rate.trim() !== ""
              ? Number(form.gas_price_rate) / 100
              : 0.05,
    ...
    if (currentCoin.value?.coin_type === "base") {
      ...
      const params = {
        ...
        coin_config: {
          coin_type: currentCoin.value.coin_type,
          contract_address:
              currentCoin.value.contract_address || null,
...
async function checkGasPriceForTransfer() {
  ...
  try {
    // 获取当前gas价格
    const res = await invoke("get_chain_gas_price", {
      chain: chainValue.value,
    });
    ...
    if (gasPrice > maxGasPrice) {
      ...
      if (
        ...
        Notification.warning({
          content: `Gas价格 ${gasPrice.toFixed(3)} Gwei 超过设定上限 ${maxGasPrice} Gwei，转账已暂停`,
          position: "topLeft",
        });
      }
      return false;
      ...
      if (transferPaused.value) {
        ...
        Notification.success({
          content: `Gas价格 ${gasPrice.toFixed(3)} Gwei 已降至设定范围内，转账将自动恢复`,
          position: "topLeft",
        });
    ...
    const config = {
      ...
      transfer_amount_list: [
        ...
        form.send_max_count && form.send_max_count.trim() !== ""
            ? Number(form.send_max_count)
            : 0,
      ], // 转账数量 (transfer_type 为 3 时生效) 转账数量在5-10之间随机，第二个数要大于第一个数！！
      left_amount_list: [
        form.send_min_count && form.send_min_count.trim() !== ""
            ? Number(form.send_min_count)
            : 0,
        form.send_max_count && form.send_max_count.trim() !== ""
            ? Number(form.send_max_count)
            : 0,
      ], // 剩余数量 (transfer_type 为 4 时生效) 剩余数量在4-6之间随机，第二个数要大于第一个数！！
      amount_precision:
          form.amount_precision && form.amount_precision.trim() !== ""
              ? Number(form.amount_precision)
              : 6, // 一般无需修改，转账个数的精确度 6 代表个数有6位小数
      limit_type: form.limit_type, // limit_type 限制类型 1：自动 2：指定数量 3：范围随机
      limit_count:
          form.limit_count && form.limit_count.trim() !== ""
              ? Number(form.limit_count)
              : 21000, // limit_count 指定数量 (limit_type 为 2 时生效)
      limit_count_list: [
        form.limit_min_count && form.limit_min_count.trim() !== ""
            ? Number(form.limit_min_count)
            : 21000,
      ...
      gas_price_rate:
          form.gas_price_rate && form.gas_price_rate.trim() !== ""
              ? Number(form.gas_price_rate) / 100
              : 0.05, // gas price溢价率，0.05代表gas price是当前gas price的105%
      gas_price:
          form.gas_price && form.gas_price.trim() !== ""
              ? Number(form.gas_price)
              : 30, // 设置最大的gas price，单位gwei
      max_gas_price:
          form.max_gas_price && form.max_gas_price.trim() !== ""
              ? Number(form.max_gas_price)
              : 0, // 设置最大的gas price，单位gwei
    };
    ...
    try {
      ...
      if (sharedConfig) {
        ...
        if (sharedConfig.chainValue) {
          ...
          const chain = chainOptions.value.find(
              (c) => c.key === sharedConfig.chainValue,
          );
          if (chain) {
            currentChain.value = chain;
          }
        ...
        if (sharedConfig.coinValue) {
          ...
          const coin = coinOptions.value.find(
              (c) => c.key === sharedConfig.coinValue,
          );
          if (coin) {
            currentCoin.value = coin;
          }
    ...
    chainOptions.value = [
      {
        key: "eth",
        name: "Ethereum",
        scan_url: "etherscan.io",
        pic_url: "eth.png",
      },
      {
        key: "bnb",
        name: "BNB Chain",
        scan_url: "bscscan.com",
        pic_url: "bnb.png",
      },
    ];
...
onMounted(async () => {
  ...
  if (isTauri) {
    try {
      ...
      console.error(
          "Error getting window title or setting close listener:",
          error,
      );
    }
  } else {
  ...
  if (isTauriMounted) {
    await listen("balance_item_update", (event) => {
      ...
      const targetIndex = data.value.findIndex(
          (dataItem) => dataItem.key === item.key,
      );
      if (targetIndex !== -1) {
        // 更新对应地址的数据（只更新指定字段）
    ...
    await listen("transfer_status_update", (event) => {
      ...
      if (item && item.private_key) {
        targetIndex = data.value.findIndex(
            (dataItem) => dataItem.private_key === item.private_key,
        );
      } else {
        ...
        try {
          ...
          for (let i = 0; i < totalBatches; i++) {
            ...
            const {validItems, invalidItems} =
                await processBatchData(batchData, i, totalBatches);
  ...
  if (chainResult.length > 0) {
    ...
    try {
      ...
      if (isTauri) {
        // 检查是否有启用的RPC节点
        try {
          const rpcProviders = await invoke("get_rpc_providers", {
            chainKey: chainValue.value,
          });
          const activeRpcs = rpcProviders.filter(
              (rpc) => rpc.is_active,
          );
...
async function coinChange(value) {
  currentCoin.value = coinOptions.value.filter(
      (item) => item.key === value,
  )[0];
}
...
function handleWalletImportConfirm(importData) {
  ...
  if (duplicateKeysCount > 0 || duplicateAddressesCount > 0) {
    ...
    if (duplicateKeysCount > 0)
      duplicateInfo.push(`${duplicateKeysCount}个重复私钥`);
    if (duplicateAddressesCount > 0)
      duplicateInfo.push(`${duplicateAddressesCount}个重复地址`);
...
async function queryFromAddressBalance(item) {
  try {
    ...
    const coinSymbol =
        currentCoin.value?.coin_symbol ||
        (currentCoin.value?.coin_type === "base" ? "ETH" : "代币");
    const walletShort = item.private_key
        ? item.private_key.substring(0, 8) + "..."
...
async function queryBalanceInBatches() {
  ...
  console.log(
      `开始分批查询余额，总数: ${totalItems}, 批次数: ${totalBatches}, 每批大小: ${BATCH_SIZE}`,
  );
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      console.log(
          `执行第 ${batchIndex + 1}/${totalBatches} 批，索引 ${startIndex}-${endIndex - 1}`,
      );
      ...
      if (balanceStopFlag.value && !shouldStopAtStart) {
        console.log(
            `第 ${batchIndex + 1} 批完成后检测到停止信号，停止后续查询`,
        );
        stoppedMidway = true;
        break;
      }
      ...
      result.items.forEach((resultItem, index) => {
        ...
        if (data.value[dataIndex]) {
          ...
          const originalPrivateKey =
              data.value[dataIndex].private_key;
...
async function queryToAddressBalance() {
  ...
  // 检查是否有到账地址
  const itemsWithToAddr = JSON.parse(
      JSON.stringify(data.value.filter((item) => item.to_addr)),
  );
...
async function queryToAddressBalanceInBatches() {
  ...
  console.log(
      `开始分批查询到账地址余额，总数: ${totalItems}, 批次数: ${totalBatches}, 每批大小: ${BATCH_SIZE}`,
  );
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      console.log(
          `执行第 ${batchIndex + 1}/${totalBatches} 批到账地址，索引 ${startIndex}-${endIndex - 1}`,
      );
      ...
      if (balanceStopFlag.value && !shouldStopAtStart) {
        console.log(
            `第 ${batchIndex + 1} 批到账地址查询完成后检测到停止信号，停止后续查询`,
        );
        stoppedMidway = true;
        break;
      }
  ...
  const performValidationAndStart = () => {
    try {
      ...
      if (!quickValidation.isValid) {
        startLoading.value = false;
        Notification.warning(
            quickValidation.reason === "存在私钥或地址为空的数据"
                ? "请检查是否所有私钥都有对应的转账地址！"
                : "包含转账金额为空的错误数据请核实！",
        );
        return;
...
function handleTransferConfirmOk() {
  ...
  setTimeout(() => {
    // 继续上次转账 - 只处理等待执行的项目
    const incompleteData = data.value.filter(
        (item) => item.exec_status === "0",
    );
  ...
  validateForm()
      .then(async () => {
        ...
        if (resetStatus) {
          ...
          console.log(
              "转账开始时间:",
              new Date(transferStartTime.value).toLocaleString(),
          );
        }
...
async function iterTransfer(accountData) {
      ...
      if (index < accountData.length - 1 && !stopFlag.value) {
        ...
        if (nextPendingIndex !== -1) {
          ...
          const countdownInterval = setInterval(() => {
            // 只在待执行状态时更新倒计时
            if (
                data.value[nextPendingIndex] &&
                data.value[nextPendingIndex].exec_status === "0"
            ) {
              data.value[nextPendingIndex].error_msg =
                  `等待中...${remainingTime}秒`;
            }
            ...
            const checkStopFlag = () => {
              if (stopFlag.value) {
                ...
                if (
                    nextPendingIndex !== -1 &&
                    data.value[nextPendingIndex] &&
                    data.value[nextPendingIndex].exec_status ===
                    "0"
                ) {
                  data.value[nextPendingIndex].error_msg =
                      originalErrorMsg;
  ...
  accountData.forEach((item, index) => {
    ...
    walletGroups
        .get(walletAddress)
        .push({...item, originalIndex: index, realIndex: realIndex});
  });
...
async function iterTransferFuryMode(accountData) {
  ...
  accountData.forEach((item, index) => {
    ...
    walletGroups
        .get(walletAddress)
        .push({...item, originalIndex: index, realIndex});
  });
    ...
    for (const item of walletTransactions) {
      ...
      try {
        ...
        if (res && res.success && res.tx_hash) {
          ...
          console.log(
              `[狂暴模式] 交易已提交: ${realIndex + 1}, hash: ${res.tx_hash}`,
          );
        } else {
          // 提交失败
          data.value[realIndex].exec_status = "3";
  ...
  console.log(
      `[狂暴模式] 提交阶段完成，待确认交易数: ${pendingTransactions.length}`,
  );
  ...
  if (pendingTransactions.length > 0) {
    ...
    while (
      ...
      retryCount++;
      console.log(
          `[狂暴模式] 确认轮次: ${retryCount}, 待确认: ${pendingTransactions.length}`,
      );
    ...
    if (pendingTransactions.length > 0) {
      console.log(
          `[狂暴模式] ${pendingTransactions.length} 笔交易确认超时`,
      );
...
function quickValidateData() {
  ...
  if (
      dataValidationCache.value.lastDataLength === currentDataLength &&
...
async function resetDataStatusAsync() {
  ...
  for (let i = 0; i < totalItems; i += batchSize) {
    ...
    if (totalItems > 2000 && processedCount % 1000 === 0) {
      console.log(
          `数据重置进度: ${processedCount}/${totalItems} (${Math.round((processedCount / totalItems) * 100)}%)`,
      );
    }
  }
}
...
function applyAdvancedFilter() {
  ...
  // 显示筛选结果
  Notification.success({
    content: `筛选完成，共选中 ${filteredItems.length} 条数据`,
    position: "topLeft",
  });
}
...
async function submitTokenForm() {
  try {
    ...
    if (
        tokenForm.type === "token" &&
        (!tokenForm.contract_address || !tokenForm.contract_address.trim())
    ) {
      Notification.warning({
        content: "代币类型为token时，合约地址不能为空",
        position: "topLeft",
      });
      return false;
    }
    if (
        tokenForm.type === "token" &&
        (!tokenForm.abi || !tokenForm.abi.trim())
    ) {
      Notification.warning({
        content: "代币类型为合约代币时，ABI不能为空",
        position: "topLeft",
      });
      return false;
    }
...
async function handleChainUpdated() {
  ...
  try {
    ...
    if (isTauri) {
      ...
      // 检查当前选中的链是否还存在
      const currentChainExists = chainOptions.value.find(
          (chain) => chain.key === chainValue.value,
      );
...
<template>
  ...
  >
    ...
    >
      <!-- 骨架屏 -->
      <TableSkeleton
          v-if="(tableLoading || balanceLoading) && data.length === 0"
          :rows="8"
      />
            ...
            >
              <template #content>
                <div
                    class="exec-actions"
                    @mouseenter="
                                        rowHoverStates[record.key] = true
                                    "
                    @mouseleave="
                                        rowHoverStates[record.key] = false
                                    "
                >
                  <div
                      class="action-btn"
              ...
              <a-tag
                  v-if="record.exec_status === '1'"
                  color="#ff7d00"
              >执行中
              </a-tag>
              <a-tag
                  v-if="record.exec_status === '2'"
                  color="#00b42a"
              >执行成功
              </a-tag>
              <a-tag
                  v-if="record.exec_status === '3'"
                  color="#f53f3f"
              >执行失败
              </a-tag>
            </a-tooltip>
      ...
      <div v-if="showImportProgress" class="floating-progress-bar">
        <div class="progress-content">
          <div class="progress-header">
            ...
            >{{ importCompleted }} / {{ importTotal }}</span
            >
          </div>
          <a-progress
    ...
    <Transition name="progress-slide" appear>
      <div
          ...
          :style="{ top: showImportProgress ? '120px' : '45px' }"
      >
        <div class="progress-content">
          <div class="progress-header">
            <span class="progress-title">转账进度</span>
            <span class="progress-count"
            >{{ transferCompleted }} / {{ transferTotal }}</span
            >
          </div>
          <a-progress
    ...
    <Transition name="progress-slide" appear>
      <div
          ...
          :style="{
                    top:
                        showImportProgress && showProgress
                            ? '220px'
                            : showImportProgress || showProgress
                              ? '120px'
                              : '45px',
      ...
      >
        <div class="progress-content">
          <div class="progress-header">
            ...
            <span class="progress-count"
            >{{ balanceCompleted }} / {{ balanceTotal }}</span
            >
          </div>
          <a-progress
    ...
    <Transition name="progress-slide" appear>
      <div
          v-if="showToAddressBalanceProgress"
          class="floating-progress-bar"
          :style="{
                    top:
                        showImportProgress &&
                        ...
                        showBalanceProgress
                            ? '320px'
                            : (showImportProgress && showProgress) ||
                                (showImportProgress && showBalanceProgress) ||
                                (showProgress && showBalanceProgress)
                              ? '220px'
                              : showImportProgress ||
                                  showProgress ||
                                  showBalanceProgress
        ...
        <div class="progress-content">
          <div class="progress-header">
            ...
            >{{ toAddressBalanceCompleted }} /
                            {{ toAddressBalanceTotal }}</span
            >
          </div>
          <a-progress
              :percent="toAddressBalanceProgress"
              :show-text="true"
              :stroke-width="6"
              :color="{
                            '0%': '#52c41a',
    ...
    >
      ...
      >
        <a-spin size="small"/>
        <span style="font-size: 14px; color: #1d2129; font-weight: 500"
        >智能重试检查中...</span
        >
      </div>
    ...
    <!-- 智能重试结果显示 -->
    <div
        v-if="retryResults.length > 0 && !retryInProgress"
        style="
                margin-top: 10px;
                padding: 10px;
                background: #f6ffed;
    ...
    >
      ...
      <div style="font-size: 12px; color: #52c41a; margin-bottom: 4px">
        跳过重试:
        {{ retryResults.filter((r) => r.action === "跳过重试").length }}
        笔 | 加入重试:
        {{ retryResults.filter((r) => r.action === "加入重试").length }}
        笔
      </div>
    </div>
      ...
      >
        <a-row style="display: flex; gap: 10px">
          ...
          <div style="flex: 10">
            ...
            <a-form-item
                v-if="
                                form.send_type === '3' || form.send_type === '4'
                            "
          ...
          <div style="flex: 9">
            ...
            <a-form-item
                v-if="
                                form.gas_price_type === '1' ||
                                form.gas_price_type === '3'
                            "
                field="max_gas_price"
                label="最大 Gas Price"
                :label-col-props="{ span: 7 }"
                ...
                >
                  <div @click="debouncedStopTransfer">
                    ...
                    <a-button
                        ...
                        class="core-action-btn success-btn stopping"
                        loading
                    >
                      <template #icon>
                        <Icon icon="mdi:timer-sand"/>
  ...
  <!-- 添加代币弹窗 -->
  <a-modal
      v-model:visible="addCoinVisible"
      :width="700"
      title="添加代币"
      @cancel="handleAddCoinCancel"
      :on-before-ok="handleAddCoinBeforeOk"
      unmountOnClose
  >
    <a-input
        v-model="coinAddress"
  ...
  <a-modal v-model:visible="deleteItemVisible" title="删除确认">
    ...
    <template #footer>
      ...
      <a-button
          type="primary"
          status="danger"
          @click="debouncedDeleteItemConfirm"
          style="margin-left: 10px"
      >确定
      </a-button>
    </template>
    ...
    <a-form :model="filterForm" layout="vertical">
      ...
      <a-form-item label="平台币余额筛选">
        <div style="display: flex; gap: 10px; align-items: center">
          <a-select
              v-model="filterForm.platBalanceOperator"
              style="width: 100px"
          >
          ...
          <a-input
              v-model="filterForm.platBalanceValue"
              placeholder="请输入平台币余额值"
              style="flex: 1"
      ...
      <a-form-item label="代币余额筛选">
        <div style="display: flex; gap: 10px; align-items: center">
          <a-select
              v-model="filterForm.coinBalanceOperator"
              style="width: 100px"
          >
            <a-option value="gt">大于</a-option>
            <a-option value="eq">等于</a-option>
          ...
          <a-input
              v-model="filterForm.coinBalanceValue"
              placeholder="请输入代币余额值"
              style="flex: 1"
              @input="debouncedFilterUpdate"
          />
        </div>
      ...
      <a-form-item label="错误信息">
        <a-input
            v-model="filterForm.errorMsg"
            placeholder="请输入要匹配的错误信息"
            @input="debouncedFilterUpdate"
        />
      </a-form-item>
    </a-form>
    ...
    <div class="tip-modal-content">
      ...
      <div class="tip-info">
        <div class="tip-info-row">
          ...
          <span class="tip-label" style="margin-left: 16px"
          >币种:</span
          >
      ...
      <div v-if="showPrivateKeyInput" class="tip-private-key-section">
        ...
        <div class="private-key-input-area">
          ...
          <!-- 私钥验证状态 -->
          <div
              v-if="shouldShowTipWalletStatus"
              class="tip-wallet-status"
          >
            <div
                v-if="tipWalletBalance.loading"
                class="wallet-info-loading"
            >
              <Icon
                  icon="mdi:loading"
                  class="loading-icon"
            ...
            <div
                v-else-if="tipWalletBalance.valid"
                class="wallet-info-valid"
            >
              <div class="wallet-address">
                ...
                />
                {{
                  tipWalletBalance.address?.substring(0, 10)
                }}...{{ tipWalletBalance.address?.slice(-8) }}
              </div>
              <div
                  class="wallet-balance"
                  :class="{ insufficient: !tipBalanceSufficient }"
              >
                <Icon
                    icon="mdi:coins"
                ...
                />
                {{
                  currentCoin?.coin_type === "base"
                      ? "平台币"
                      : "代币"
                }}余额: {{ tipWalletBalance.balance }}
                {{ currentCoin?.symbol || "Token" }}
              </div>
              <div
                  v-if="tipAmount && !tipBalanceSufficient"
            ...
            <div
                v-else-if="
                                ...
                                tipWalletBalance.hasAttempted
                            "
                class="wallet-info-invalid"
            >
              <Icon
      ...
      <div class="tip-mode-switch">
        <a-button
            v-if="showQRCode"
            type="outline"
            @click="switchTipMode('privatekey')"
        ...
        <a-button
            v-if="showPrivateKeyInput"
            type="outline"
            @click="switchTipMode('qrcode')"
            class="switch-mode-btn"
        >
    ...
    <template #footer>
      <div class="tip-footer">
        ...
        <a-button
            ...
            :loading="tipLoading"
            :disabled="
                        !tipWalletBalance.valid ||
                        !tipBalanceSufficient ||
                        !tipAmount ||
                        tipWalletBalance.loading
                    "
            size="large"
  ...
  >
        ...
        >
          ...
          <template #option="{ data }">
            ...
            >
              ...
              />
              <span>{{ data?.name }}</span>
              <span style="color: #86909c; margin-left: auto">{{
                  data?.scan_url
                }}</span>
            </div>
          </template>
    ...
    <template #footer>
      ...
      <a-button type="primary" @click="handleSelectionModalOk"
      >确定
      </a-button
      >
    </template>
  ...
  <div
      ...
      :class="{ 'with-progress': showImportProgress }"
  >
    ...
    <div class="status-bar-left">
      ...
      >
        <Icon
            ...
            style="
                        font-size: 14px;
                        color: var(--text-color-tertiary, #c9cdd4);
                    "
        />
...
.core-action-btn.primary-btn {
  background: linear-gradient(135deg, #1a6fff 0%, #4d8aff 50%, #6b5fff 100%);
  box-shadow: 0 6px 20px rgba(26, 111, 255, 0.45), 0 2px 8px rgba(26, 111, 255, 0.2);
}

.core-action-btn.primary-btn:hover {
  background: linear-gradient(135deg, #4d8aff 0%, #6b5fff 50%, #8b7fff 100%);
  box-shadow: 0 10px 30px rgba(26, 111, 255, 0.55), 0 4px 12px rgba(26, 111, 255, 0.3);
}

.core-action-btn.executing.primary-btn {
  background: linear-gradient(135deg, #4d8aff 0%, #6b5fff 100%);
  box-shadow: 0 6px 20px rgba(26, 111, 255, 0.45), 0 2px 8px rgba(26, 111, 255, 0.2);
  animation: pulse-primary-soft 2s ease-in-out infinite;
}
...
@keyframes pulse-primary-soft {
  ...
  50% {
    box-shadow: 0 6px 35px rgba(26, 111, 255, 0.7), 0 3px 12px rgba(26, 111, 255, 0.35);
  }
}

.core-action-btn.executing.primary-btn:hover {
  background: linear-gradient(135deg, #e63946 0%, #ff6b6b 100%);
  box-shadow: 0 10px 30px rgba(230, 57, 70, 0.5), 0 4px 12px rgba(230, 57, 70, 0.25);
  animation: none;
}

.core-action-btn.stopping.primary-btn {
  background: linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%);
  box-shadow: 0 6px 20px rgba(245, 158, 11, 0.45), 0 2px 8px rgba(245, 158, 11, 0.2);
}
...
.execute-btn:hover {
  color: #ffffff;
  transform: translateY(-3px) scale(1.02);
  box-shadow: 0 12px 35px rgba(16, 168, 92, 0.55), 0 4px 15px rgba(16, 168, 92, 0.3);
  background: linear-gradient(135deg, #12c47d 0%, #14e08e 50%, #16f89f 100%);
}
...
.execute-btn.executing {
  background: linear-gradient(135deg, #12c47d 0%, #14e08e 100%);
  box-shadow: 0 6px 20px rgba(16, 168, 92, 0.45), 0 2px 8px rgba(16, 168, 92, 0.2);
  animation: pulse-execute-btn 2s ease-in-out infinite;
}

@keyframes pulse-execute-btn {
  0%, 100% {
    box-shadow: 0 6px 20px rgba(16, 168, 92, 0.45), 0 2px 8px rgba(16, 168, 92, 0.2);
  }
  50% {
    box-shadow: 0 6px 40px rgba(16, 168, 92, 0.75), 0 4px 15px rgba(16, 168, 92, 0.4);
  }
}
...
.execute-btn.executing:hover {
  color: #ffffff;
  background: linear-gradient(135deg, #e63946 0%, #ff6b6b 100%);
  box-shadow: 0 12px 35px rgba(230, 57, 70, 0.55), 0 4px 15px rgba(230, 57, 70, 0.3);
  animation: none;
}
...
.core-action-btn.primary-btn .arco-icon {
  filter: brightness(1.2) drop-shadow(0 1px 2px rgba(0,0,0,0.2));
}

.core-action-btn.success-btn .arco-icon {
  filter: brightness(1.15) drop-shadow(0 1px 2px rgba(0,0,0,0.2));
}

.execute-btn .arco-icon {
  transition: transform 0.3s ease, opacity 0.3s ease;
  filter: drop-shadow(0 1px 2px rgba(0,0,0,0.2));
}

.execute-btn:hover .arco-icon {
  transform: scale(1.15);
}

.execute-btn.executing .arco-icon {
