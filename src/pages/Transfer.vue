...
async function rpcChange() {
  ...
  currentRpc.value = rpcOptions.value.filter(
      (item) => item.key === rpcValue.value
  )[0];
        ...
        .then((proxy_address) => {
          ...
          token_utils
              ...
              .then((abi) => {
                console.log("获取代币名称");
                token_utils
                    .getTokenSymbol(rpcValue.value, coinAddress.value, abi)
                    .then((symbol) => {
                      let json = {
...
async function queryBalance() {
  ...
  if (currentCoin.value.type === "base" || currentCoin.value.type === "token") {
    ...
    balance_utils.exec_group_query(
        rpcValue.value,
        currentCoin.value,
        data.value,
        () => {
  ...
  for (let i = 0; i < accountData.length; i++) {
    try {
      const config = {
        ...
        chain: rpcValue.value,
        chainLayer: currentRpc.value.layer,
        l1: currentRpc.value.l1,
    ...
    <div class="mainTable">
      ...
      >
        ...
        <template #exec_status="{ rowIndex }">
          <a-tag v-if="data[rowIndex].exec_status === '0'" color="#86909c"
          >等待执行
          </a-tag
          >
          <a-tag v-if="data[rowIndex].exec_status === '1'" color="#ff7d00"
          >执行中
          </a-tag
          >
          <a-tag v-if="data[rowIndex].exec_status === '2'" color="#00b42a"
          >执行成功
          </a-tag
          >
          <a-tag v-if="data[rowIndex].exec_status === '3'" color="#f53f3f"
          >执行失败
          </a-tag
          >
        </template>
        <template #optional="{ record }">
          <icon-delete
              ...
              @click.prevent="deleteItem(record)"
          />
        </template>
      </a-table>
    </div>
      ...
      >
        <a-row style="height: 80px">
          <a-form-item
              field="send_type"
              label="发送方式"
              style="width: 315px; padding: 10px"
          >
        ...
        <a-row v-show="rpcValue !== 'sol'" style="height: 80px">
          ...
          <a-form-item
              ...
              style="width: 265px; padding: 10px"
      ...
      >
        ...
        <a-tooltip v-else content="点击可以提前停止执行">
          <div @click="stopTransfer">
            <a-button v-if="!stopFlag" class="submitBtn" loading
            >执行中...
            </a-button
            >
            <a-button v-if="stopFlag && !stopStatus" class="submitBtn" loading
            >正在停止...
            </a-button
            >
          </div>
